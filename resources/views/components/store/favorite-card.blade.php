...
<article class="group flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-love-pink-100/80 bg-white shadow-[0_22px_50px_-34px_rgba(15,23,42,0.34)] transition duration-300" data-favorite-card data-favorite-slug="{{ $slug }}">
    <div class="relative overflow-hidden bg-slate-100">
        <a class="block" href="{{ $href }}" aria-label="View {{ $title }} details">
    ...
    <div class="flex flex-1 flex-col p-5">
        <h3 class="text-xl font-extrabold text-slate-950">
            <a class="transition hover:text-love-pink-500" href="{{ $href }}">{{ $title }}</a>
        </h3>
        <div class="mt-3 flex items-center justify-between gap-4">
            <p class="text-2xl font-extrabold text-love-orange-500">&#8369;{{ number_format((float) $price) }}</p>
