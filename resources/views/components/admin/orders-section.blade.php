...
<section class="grid gap-6" data-admin-order-management data-backend-orders="true">
    ...
    <script>
        (() => {
            const section = document.querySelector('[data-admin-order-management][data-backend-orders="true"]');
            ...
            section.querySelectorAll('[data-admin-details-open]').forEach((button) => {
                button.addEventListener('click', () => {
                    const template = document.getElementById(button.dataset.detailsTemplate || '');

                    if (!(template instanceof HTMLTemplateElement) || !detailsContent) {
                        return;
                    }
                    ...
                    detailsContent.append(template.content.cloneNode(true));
