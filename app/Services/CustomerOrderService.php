...
class CustomerOrderService
{
     ...
     */
    public function activeOrderCardsFor(User $user): Collection
    {
        return Order::query()
            ->whereBelongsTo($user)
     ...
     * @return Collection<int, array<string, mixed>>
     */
    public function deliveredOrderCardsFor(User $user): Collection
    {
        return Order::query()
            ->whereBelongsTo($user)
            ->where('status', Order::STATUS_DELIVERED)
    ...
    public function confirmDelivery(Order $order, User $user): void
    {
        ...
        if ($order->status !== Order::STATUS_OUT_FOR_DELIVERY) {
            throw ValidationException::withMessages([
                'order' => 'This order can only be confirmed once it is out for delivery.',
            ]);
        }
